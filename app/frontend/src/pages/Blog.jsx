  ...
  if (error || blogPosts.length === 0) {
    return (
      <div className="text-center py-32 px-6">
        <h2 className="text-2xl font-black mb-4">{error || 'No blog posts found'}</h2>
        <p className="text-on-surface-variant mb-8">Please check back later for exciting new content.</p>
      </div>
    );
  }
        ...
        {blogPosts.length > 0 && (
          <div className="mb-12">
            <Link to={`/blog/${blogPosts[0].id}`} className="relative rounded-3xl overflow-hidden group shadow-md hover:shadow-xl transition-shadow cursor-pointer block h-[400px] md:h-[500px]">
              <img src={blogPosts[0].image_url} alt={blogPosts[0].title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent flex flex-col justify-end p-8 md:p-12">
                <span className="bg-primary text-on-primary text-xs font-bold px-3 py-1 rounded-full w-max mb-4 uppercase tracking-widest shadow-lg shadow-primary/20">{blogPosts[0].category}</span>
                <h2 className="text-3xl md:text-4xl lg:text-5xl font-black text-white mb-4 leading-tight font-display max-w-3xl">{blogPosts[0].title}</h2>
                <p className="text-white/80 mb-6 max-w-2xl text-lg line-clamp-2 md:line-clamp-none">{blogPosts[0].excerpt}</p>
          ...
          {blogPosts.slice(1).map((post) => (
            <Link key={post.id} to={`/blog/${post.id}`} className="bg-surface-container-lowest rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group flex flex-col border border-surface-container-low cursor-pointer hover:-translate-y-1">
              ...
              <div className="p-8 flex flex-col flex-grow">
                <h3 className="text-xl font-bold text-on-surface mb-3 leading-tight group-hover:text-primary transition-colors line-clamp-2">{post.title}</h3>
                <p className="text-on-surface-variant text-sm mb-6 line-clamp-3 leading-relaxed flex-grow">{post.excerpt}</p>
